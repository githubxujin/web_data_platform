<template>
    <div>
        <c-box type="search">
            ...
            <el-form :inline='true' ref='form' size='mini' class="data-left">
                ...
                <el-form-item label='药品名称'>
                    <el-select v-model='params.productName' clearable multiple :multiple-limit="max" @change="changeProductName" collapse-tags style="margin-left: 20px; width: 260px" filterable remote
                        :remote-method="remoteProductName" :loading="loading">
                        <el-option :label='item' :value='item' v-for="(item, index) in productNameList" :key="index">
                        </el-option>
                    </el-select>
                </el-form-item>
                <el-form-item label='规格'>
                    <el-select v-model='params.productSpec' clearable filterable remote :remote-method="remoteProductSpec" :loading="loading">
        ...
        <div class="echart-big-wrap">
            <div class="echart-big-title">药品的订单量变化趋势</div>
            <chart height="511px" width="1168px" :option="drugOrderNumber" id="drugOrderNumber" />
        </div>
        <div class="echart-big-wrap">
            <div class="echart-big-title">药品销售量变化趋势</div>
            <chart height="511px" width="1168px" :option="drugSaleNumber" id="drugSaleNumber" />
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        getDrugOrderAmout () {
            this.$http("dcp/drug/purchase/quantity", this.params).then(res => {
                ...
                drawOpt.yAxis[0].name = "";
                drawOpt.legend.data = drugData.map(i => i.productName);
                drugData.forEach(i =>
                    drawOpt.legend.data.push(i.productName + "增长率")
                );
                this.drugOrderNumber = drawOpt;
            });
        },
        getDrugSaleAmout () {
            this.$http("/dcp/drug/sale/quantity", this.params).then(res => {
                let drugData = res.data;
                let drawOpt = deepClone(overlapOption);
