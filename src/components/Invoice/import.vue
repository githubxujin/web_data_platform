...
export default {
    ...
    data() {
        var validateCustomer = (rule, value, callback) => {
            if (value === "") {
                ...
                this.$http("/invoice/customer/name/verify", this.form, {
                    type: "formdata"
                }).then(res => {
                    if (res.code === "200") {
                        // result: 0 不能使用， 1 可以使用
                        if (res.data.result === 0) {
                            callback(new Error("客户名称与其所属板块不匹配!"));
                        } else {
                            this.isValid = true;
                            callback();
                        }
    ...
    methods: {
        ...
        submitForm() {
            ...
            console.log(this.form.files, "files");
            this.$refs["form"].validate(valid => {
                if (valid) {
                    let url = this.isPDF
                        ? "/zuul/invoice/offlineInvoice/check/save"
                        : "/zuul/invoice/offline/invoice/import";
                    this.$http(
                        url,
                        {
                            params: {
                                customerName: this.form.customerName,
                                industryCategory: this.form.industryCategory,
