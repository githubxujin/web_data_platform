<template>
    <div>
        ...
        <div class="echart-big-wrap p-re ">
            <div class="echart-big-title num-left">开票金额</div>
...
import { dayList, monthList } from "./config/date.js"
import { numFormat } from "@/filters/index";
import { getDaysValue } from "@/utils/common";
export default {
    components: {
        Chart,
        cOp
    ...
    methods: {
        ...
        getChart() {
            ...
            this.$http("/dcp/count/invoice/material/day", this.params).then(res => {
                ...
                industryTrends.yAxis.name = '单位：元',
                    ...
                    industryTrends.tooltip.formatter = function (params) {
                        ...
                        params.forEach(v => {
                            name = `${v.name}<br/>`
                            str += `${v.marker}${v.seriesName + '：'}${numFormat(v.value)} 元<br/>`
                        })
                        return name + str
                    }
                industryTrends.tooltip.formatter = function (params) {
                ...
                // let dateArr = res.data.length > 0 ? res.data[0].invoiceMaterialVoList.map(i => this.params.dateType === 1 ? i.invoiceDate + '日' : (this.params.dateType === 2 ? i.invoiceDate + '月' : i.invoiceDate + '年')) : [];
                let dateArr = []
                // 补全日期
                let arrDay = []
                ...
                if (this.params.dateType !== 3) {
                    res.data.forEach(v => {
                        v.invoiceMaterialVoList = getDaysValue(arrDay, v.invoiceMaterialVoList, 'totalAmount')
                    })
                } else {
                    res.data.forEach(i => {
                        let childrenList = []
                ...
                if (this.flag === 'light') {
                    ...
                    colorList = this.$service.output.application.industryTrends.colorList
                    industryTrends.color = colorList
                    ...
                    industryTrends.xAxis.axisLabel = {
                    ...
                    industryTrends.yAxis.axisLabel.textStyle = {
                        color: this.$medicine.nameColor,
                    }
                }
