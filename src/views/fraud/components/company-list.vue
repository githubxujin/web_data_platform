<template>
    <div class="stat">
        <el-row class="pt20">
            <el-form :inline="true" ref="form" size="medium">
                ...
                <el-form-item>
                    <el-select v-model="params.businessCategory" placeholder="企业类别" clearable>
                        <!-- <el-option label="核心&融资企业" value="0"></el-option> -->
            ...
            <div class="add-export">
                <el-tooltip class="item" effect="dark" content="新增" placement="top">
                    <span class="el-icon-plus" @click="$router.push('/fraudPanel/namesList/addNames')"></span>
                </el-tooltip>
            ...
            <el-table :data="list">
                ...
                <el-table-column prop="customerName" label="企业名称" min-width="220" show-overflow-tooltip></el-table-column>
                <el-table-column prop="industryCategory" label="行业类别" min-width="120" show-overflow-tooltip>
                    <template slot-scope="item">
                        {{ industryCategoryList[item.row.industryCategory] }}
                    </template>
                </el-table-column>
                <el-table-column prop="businessCategory" label="企业类别" min-width="120" show-overflow-tooltip>
                    <template slot-scope="item">
                ...
                <el-table-column prop="" label="操作" width="130" align="center" fixed="right">
                    <template slot-scope="item">
                        <el-button v-if="codeList[item.row.status]!=='否决'" type="blue" plain :disabled="item.row.status === 2" size="mini" @click="$router.push('/fraudPanel/namesList/amendNames/' + item.row.id)">修改
                        </el-button>
                        <el-button type="text" plain size="mini" @click="$router.push('/fraudPanel/namesList/namesDetail/' + item.row.id)">详情</el-button>
                    </template>
                </el-table-column>
            </el-table>
        ...
        <el-dialog title="导入企业名单" :center="true" v-if="isDialog" :visible.sync="isDialog" custom-class="daoru" :close-on-click-modal="false" width="470px">
            <p class="tishi">
                <i class="iconfont icon-ERPqiyeduan-tishi"></i>
                导入数据上传的附件有格式要求，点击下载
                <a class="a-corlor" href="/导入企业名单模板.xlsx" target="_blank">导入模版</a>
            </p>
            <div class="shangc">
                <span class="wen">
                    上传附件
...
export default {
    ...
    methods: {
        ...
        onSubmit () {
            ...
            // }
            this.ajax.upload('/risk/server/antiFraud/customer/import', {                file: {
