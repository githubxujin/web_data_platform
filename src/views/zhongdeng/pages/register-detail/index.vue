<template>
    <div class="page">
        <div class="mainContent">
            <div class="mod pb0">
                <el-form :inline="true" size="medium">
                    ...
                    <el-form-item>
                        <el-select v-model="params.registerType" placeholder="登记种类" clearable>
                    ...
                    <el-form-item>
                        <el-select v-model="params.registerStatus" placeholder="登记状态" clearable>
                            <el-option label="待登记" value="0"></el-option>
                            <el-option label="登记中" value="1"></el-option>
                            <el-option label="登记成功" value="2"></el-option>
                            <el-option label="登记失败" value="3"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item>
                    ...
                    <el-form-item clearable>
                        <el-select v-model="params.dealType" placeholder="交易类型">
            ...
            <div class="mod">
                <!-- 表格 -->
                <!-- <el-button class="mt10 mb30" class="el-btn-reset"  size="medium" @click="isDialog=true">新增</el-button> -->
                ...
                <el-table :data="list" style="width: 100%">
                    ...
                    <el-table-column prop="registerStatus" label="登记状态" width="100">
                        <template slot-scope="scope">
                            <span v-if="scope.row.registerStatus!='3'">{{initregisterStatus[scope.row.registerStatus]}}</span>
                            <a style="cursor: pointer" v-else @click="handlefailDesc(scope.row.registerFailDesc)">{{initregisterStatus[scope.row.registerStatus]}}</a>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="relatedCertNo" label="关联的初始登记证明编号" width="200" :show-overflow-tooltip="true">
                    </el-table-column>
                    ...
                    <el-table-column prop="subDealType" label="质押类型" width="240" :show-overflow-tooltip="true">
                    </el-table-column>
                    <el-table-column prop="createBy" label="操作者" width="120" :show-overflow-tooltip="true">
                    </el-table-column>
                    <el-table-column prop="createTime" label="操作时间" width="160" :show-overflow-tooltip="true">
                    </el-table-column>
                    <el-table-column prop="updateBy" label="更新者" width="120" :show-overflow-tooltip="true">
                    </el-table-column>
        ...
        <el-dialog v-if="descDialog" title="质押/转让财产描述" :center="true" :visible.sync="descDialog" width="700px" :close-on-click-modal="false">
            <div class="c_W">
                {{descDetail}}
            </div>
        </el-dialog>
        <el-dialog v-if="failDia" title="失败原因" :center="true" :visible.sync="failDia" width="600px">
            <div class="c_W">
                {{descfailDetail}}
            </div>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="failDia=false">确定</el-button>
            </div>
        </el-dialog>

        <el-dialog v-if="isDialog" title="详情" :visible.sync="isDialog" :close-on-click-modal="false" width="90%">
            <detail :id="curID" />
        </el-dialog>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        async initData (type) {
            ...
            if (this.params.registerStatus) {
                if (typeof (this.params.registerStatus) === 'string') {
                    states = [this.params.registerStatus]
                }
            } else {
                states = []
            }

            data = { ...this.params, ...{ registerStatus: states } }

            let res = await this.ajax.json('dcp/asset/register/page/list', data)
            if (res.code === '200') {
        ...
        handlefailDesc (val) {
            this.failDia = true
            this.descfailDetail = val
        },
        onDetail (row) {
            // this.isDialog = true
            this.curID = row.registerId
            this.$router.push({
                path: '/zhongdeng/info-detail',
                query: { id: row.registerId }
            })
