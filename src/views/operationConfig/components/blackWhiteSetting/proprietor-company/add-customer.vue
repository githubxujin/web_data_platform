<template>
    <div>
        <el-dialog :title="dialogTitle($parent.isEdit)" :visible.sync="$parent.dialogVisible" style="margin-top: -10vh;" width="1060px">
            <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="140px" class="demo-ruleForm form_content" label-position="left" style="margin: 0 28px 0 28px">
                <div class="form_content_top">
                    <div class="form_left">
                        ...
                        <el-form-item label="名单类型" prop="listType">
                            <el-select v-model="ruleForm.listType" clearable placeholder="请选择名单类型" :disabled="disabled" @change="listTypeChange" style="width: 260px">
                        ...
                        <el-form-item label="企业名称" prop="customerName">
                            <el-input v-model="ruleForm.customerName" :disabled="disabled" @change="customerNameChange" placeholder="请输入企业名称" style="width: 260px"></el-input>
                        </el-form-item>
                        <el-form-item label="统一社会信用代码" prop="unifiedCreditCode">
                            <el-input v-model="ruleForm.unifiedCreditCode" :disabled="disabled" placeholder="输入企业名称后自动生成" style="width: 260px"></el-input>
                        </el-form-item>
                        <el-form-item label="正面标签" prop="positiveLabelContent">
                            <el-select v-model="ruleForm.positiveLabelContent" clearable placeholder="请选择/输入标签" :disabled="disabled" style="width: 260px" filterable allow-create default-first-option>
                                <el-option v-for="(s, index) in positiveLabelList" :label="s" :value="s" :key="index"></el-option>
                            </el-select>
                        </el-form-item>
                        <el-form-item label="负面标签" prop="negativeLabelContent">
                            <el-select v-model="ruleForm.negativeLabelContent" clearable placeholder="请选择/输入标签" :disabled="disabled" style="width: 260px" filterable allow-create default-first-option>
                                <el-option v-for="(s, index) in negativeabelList" :label="s" :value="s" :key="index"></el-option>
                            </el-select>
                        </el-form-item>
                    </div>
                    ...
                    <div class="form_right">
                        <el-form-item label="企业类别" prop="businessCategory">
                            <el-select v-model="ruleForm.businessCategory" clearable placeholder="请选择企业类别" disabled style="width: 260px">
                        ...
                        <el-form-item label="省市">
                            <el-cascader :options="city" v-model="selectedOptions" @change="handleChange" :props="{ expandTrigger: 'hover' }" :disabled="disabled" style="width: 260px"></el-cascader>
                        </el-form-item>
                        <el-form-item label="时效性">
                            <el-date-picker v-model="dateArr" type="daterange" unlink-panels @change="changeDate" value-format="yyyy-MM-dd" range-separator="至" :disabled="disabled" style="width: 260px"></el-date-picker>
                        </el-form-item>
                        ...
                        <el-form-item label="上级单位" prop="parentUnit">
                            <el-input v-model="ruleForm.parentUnit" :disabled="disabled" placeholder="输入企业名称后自动生成" style="width: 260px"></el-input>
                        </el-form-item>
                        <el-form-item label="所在层级" prop="grade">
                            <el-select v-model="ruleForm.grade" clearable placeholder="输入企业名称后自动生成" :disabled="disabled" style="width: 260px">
                        ...
                        <el-form-item label="跟进人" prop="followPeople">
                            <el-input v-model="ruleForm.followPeople" :disabled="disabled" placeholder="请输入跟进人" style="width: 260px"></el-input>
                        </el-form-item>
                    </div>
                </div>
                ...
                <el-form-item label="动态" v-if="$parent.isEdit === 3 || $parent.isEdit === 4 ">
                ...
                <el-form-item v-if="$parent.isEdit === 4" label="审核" prop="reason">
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                parentUnit: [{ required: true, message: "上级单位不能为空", trigger: "blur" }],
                grade: [{ required: true, message: "所在层级不能为空", trigger: "blur" }],
            ...
            reason: "",  //审批意见
            parentId: "",  //父组件id
            positiveLabelList: [],  //正面标签下拉信息
            negativeabelList: [],  //负面标签下拉信息
        }
    },
    created() {
        ...
        if (this.$parent.isEdit === 1) {//新增
            this.disabled = false;
            // this.getRankedList();
        } else if (this.$parent.isEdit === 2) { //编辑
            this.disabled = false;
            this.getDetailList();
            this.getFileList();
            // this.getRankedList();
    ...
    methods: {
            ...
            if (!this.disabled) {
                this.$refs[formName].validate(valid => {
                    if (valid) {
                        ...
                        this.ruleForm.positiveLabelContent = this.ruleForm.positiveLabelContent;
                        this.ruleForm.negativeLabelContent = this.ruleForm.negativeLabelContent;
                        this.$http("/dcp/antiFraud/customer/save", this.ruleForm, {
                            type: "POST"
                        }).then(res => {
                            if (res.code === "200") {
                                if (this.$parent.isEdit === 1) {
        ...
        getLabelList(businessType) {
            this.ajax.json(
                "/dcp/server/antiFraud/label/list", { businessType: businessType, listType: this.ruleForm.listType }, { type: 'get' }
            ).then(res => {
                let data = res.data;
                if (businessType === 1) {
        ...
        changeDate(v) {
            if (v && v.length > 0) {
                this.ruleForm.startDate = v[0]
                this.ruleForm.endDate = v[1]
            } else {
        ...
        //     this.checkedId = data.parentId
        //     this.$refs.DeviceGroupTree.setCheckedNodes([data]);
        // },
        //获取企业层级信息
        // getRankedList() {
        //     this.ajax.json(
        //         "/dcp/antiFraud/company/tree/node", {}, { type: 'get' }
        //     ).then(res => {
        //         let data = res.data;
        //         data.map(first_item => {
        //             first_item.str1 = 1;
        //             if (first_item.childList && first_item.childList.length) {
        //                 first_item.childList.map(second_item => {
        //                     second_item.str1 = 1;
        //                     second_item.str2 = 2;
        //                     if (second_item.childList && second_item.childList.length) {
        //                         second_item.childList.map(third_item => {
        //                             third_item.str1 = 1;
        //                             third_item.str2 = 2;
        //                             third_item.str3 = 3;
        //                         })
