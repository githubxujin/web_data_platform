<template>
    ...
    <div class="stat">
        <div class="table-wrap mt15 mb30">
            <!-- 判断入口是增加还是修改 -->
            <h3 class="hos-detail" v-if="isAdd">器械名称(新增器械(备案)编辑)</h3>
            <h3 class="hos-detail" v-else>器械名称(器械备案之编辑)</h3>
            <table class="table-public-detail" v-if="isAdd">
                <tr>
                    <td>器械名称</td>
                    <td>
                    ...
                    <td>产品类型</td>
                    <td>
                        <el-select v-model="params.inPool" placeholder="请选择" clearable style="width:100%;">
                            <el-option label="进口"></el-option>
                            <el-option label="国产"></el-option>
                        </el-select>
                    </td>
                </tr>
            </table>
          ...
          <span>药厂基本情况</span>
          <span v-if="params.updateDate">保存于：{{params.updateDate}}</span>
        </div>-->
            ...
            <!-- <el-table :data="negativeList" border class="tab-info">
        ...
        <el-table-column prop="negativeContent" label="内容" align="center">
          <template slot-scope="item">
            ...
            <div class="to-submit mt30">
                <el-button type="primary" size="medium" @click="$router.push('/drugFactoryNum')">取消</el-button>
        ...
        <el-dialog v-if="isDialog" :visible.sync="isDialog" width="500px" title="提示" align="center" :close-on-click-modal="false">
            <p style="color: rgba(255,255,255,0.80);">已存在【{{this.params.hospitalName}}】的数据名单</p>
...
export default {
    ...
    methods: {
        ...
        toDel (n) {
            if (n === 1) {
                if (this.params.hospitalPeriodList.length > 0) {
                    this.params.hospitalPeriodList.splice(
                        this.params.hospitalPeriodList.length - 1,
                        1
                    );
                    let v = this.params.hospitalPeriodList[
                        this.params.hospitalPeriodList.length - 1
                    ].supplierName;
                ...
                if (this.params.hospitalInvoiceList.length > 0) {
                    this.params.hospitalInvoiceList.splice(
                        this.params.hospitalInvoiceList.length - 1,
                        1
                    );
                    let v = this.params.hospitalInvoiceList[
                        this.params.hospitalInvoiceList.length - 1
                    ].hospitalInvoiceName;
                ...
                if (this.params.hospitalBusinessList.length > 0) {
                    this.params.hospitalBusinessList.splice(
                        this.params.hospitalBusinessList.length - 1,
                        1
                    );
                    let v = this.params.hospitalBusinessList[
                        this.params.hospitalBusinessList.length - 1
                    ].businessYear;
                ...
                if (this.params.hospitalFinanceList.length > 0) {
                    this.params.hospitalFinanceList.splice(
                        this.params.hospitalFinanceList.length - 1,
                        1
                    );
                    let v = this.params.hospitalFinanceList[
                        this.params.hospitalFinanceList.length - 1
                    ].financeYear;
                ...
                if (this.params.hospitalLocationEconomicList.length > 0) {
                    this.params.hospitalLocationEconomicList.splice(
                        this.params.hospitalLocationEconomicList.length - 1,
                        1
                    );
                    let v = this.params.hospitalLocationEconomicList[
                        this.params.hospitalLocationEconomicList.length - 1
                    ].economicYear;
        ...
        checkRepetition (v, i, n) {
            let arr = [];
            if (n === 1) {
                if (this.params.hospitalPeriodList.length < 2)
                    return (this.isCheck1 = false);
                ...
                if (this.params.hospitalInvoiceList.length < 2)
                    return (this.isCheck2 = false);
                ...
                if (this.params.hospitalBusinessList.length < 2)
                    return (this.isCheck3 = false);
                ...
                if (this.params.hospitalFinanceList.length < 2)
                    return (this.isCheck4 = false);
                ...
                if (this.params.hospitalLocationEconomicList.length < 2)
                    return (this.isCheck5 = false);
        ...
        toSubmit (type) {
            ...
            if (
                this.params.hospitalInvoiceList.length === 0 ||
                !this.params.hospitalInvoiceList[0].hospitalInvoiceName
            ) {
            ...
            if (this.params.staffNumber && !reg.test(this.params.staffNumber)) {
                ...
                this.params.seniorStaffNumber &&
                !reg.test(this.params.seniorStaffNumber)
            ) {
                return this.$message.error("高级职工人数必须为数字");
            } else if (
                this.params.doctorStaffNumber &&
                !reg.test(this.params.doctorStaffNumber)
            ) {
                return this.$message.error("博士硕士职工人数必须为数字");
            } else if (
                this.params.functionDepartmentNum &&
                !reg.test(this.params.functionDepartmentNum)
            ) {
                return this.$message.error("职能科室个数必须为数字");
            } else if (
                this.params.clinicalDepartmentNum &&
                !reg.test(this.params.clinicalDepartmentNum)
            ) {
                return this.$message.error("临床科室个数必须为数字");
            } else if (
                this.params.technologyDepartmentNum &&
                !reg.test(this.params.technologyDepartmentNum)
            ) {
            ...
            if (this.params.hospitalBusinessList.length > 0) {
                ...
                for (const key in data) {
                    for (const v in data[key]) {
                        if (data[key][v].name) {
                            if (!reg.test(data[key][v].name))
                                return this.$message.error(data[key][v].error);
                        }
                    }
                }
            }
            ...
            if (type === "submit") {
                if (this.isAdd) {
                    // 新增提交
                    this.params.hospitalNegativeList = [
                        ...this.executorList,
                        ...this.unExecutorList,
                        ...this.documentList,
                        ...this.negativeList
                    ...
                    // 修改提交
                    this.$http("/dcp/update/hospital", this.params)
                        .then(res => {
                            this.$message.success("提交成功");
