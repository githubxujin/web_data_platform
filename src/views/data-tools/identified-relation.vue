<template>
    <div class="mt20" id="fullScreen">
        <div class="table-wrap ">
            <div class="el-title pb20">企业关联方认定分析</div>
            <div class="container">
                <el-form :inline="true" ref="form" size="medium">
                    <el-form-item style="width: 320px">
                        <el-select placeholder="企业名称" v-model="form.corporateName" clearable style="width: 320px" :remote-method="remoteMethod" filterable remote allow-create>
    ...
    methods: {
        ...
        getChart () {
            if (!this.form.corporateName) {
                this.$message.error({ message: '企业名称不能为空' })
                return
            }
            this.companyName = this.form.corporateName
            let list = this.companyList.filter(i => i.companyName === this.form.corporateName)
            let creditCode = list.length > 0 ? list[0].creditCode : ''
            this.ajax
                .json(`/dcp/query/relation/square/${creditCode}`, {}, { type: 'get' })
                .then(res => {
...
.dark .d3-box /deep/ .el-drawer__wrapper.el_drawer_content{
    right: 0 !important;
}
.light .d3-box /deep/ .el-drawer__wrapper.el_drawer_content{
    right: 0 !important;
}
</style>
